...
export function simulateAnnualBattleClash(
  ...
  // Defensive homeland advantage (Defending your own realm grants tactical knowledge and local militia support)
  const defensiveHomelandBonus = war.isDefensive ? 1.12 : 1.0;

  // Combined combat strength
  const playerBattlePower = playerTroops * tacticalAdvantage * martialEfficiency * intellectForesight * commanderBonus * personalCommandBonus * fatigueCombatMod * defensiveHomelandBonus;
  const enemyBattlePower = enemyTroops * (enemyTacticObj?.statScaling ? 1.08 : 1.0) * (0.92 + Math.random() * 0.22);

  const victoryRatio = playerBattlePower / (playerBattlePower + enemyBattlePower);
  const won = victoryRatio >= 0.48;
  ...
  const oppositionIntensity = Math.min(2.2, Math.max(0.65, enemyTroops / (playerTroops || 1)));

  if (won) {
    // Victory calculation
    const baseWinScore = playerTacticObj.bonusScoreOnWin || 20;
    scoreDelta = Math.round(baseWinScore * (victoryRatio * 1.25) * (militaryPowerScore > 1 ? 1.15 : 0.95));
    scoreDelta = Math.max(10, Math.min(45, scoreDelta));

    // Even in victories, fighting a large opposition causes real, meaningful casualties
    const basePlayerLossRate = Math.max(
      0.08,
      (0.14 * oppositionIntensity) - (character.stats.intellect / 800) - (playerTacticObj.casualtyReductionRate * 0.05) - (war.isDefensive ? 0.03 : 0)
    );
    playerCasualties = Math.min(playerTroops, Math.round(playerTroops * Math.min(0.35, basePlayerLossRate)));

    // Enemy takes heavy retreat & rout casualties
    const baseEnemyLossRate = Math.max(
      0.20,
      (0.26 + (character.stats.martial / 300) + (playerTacticObj.enemyCasualtyMultiplier - 1) * 0.15)
    );
    enemyCasualties = Math.min(enemyTroops, Math.round(enemyTroops * Math.min(0.55, baseEnemyLossRate)));
  } else {
    // Defeat calculation
    scoreDelta = -Math.round(18 * (1.25 - victoryRatio));
    scoreDelta = Math.min(-10, Math.max(-40, scoreDelta));

    // Heavy player casualties on defeat
    const basePlayerLossRate = Math.max(
      0.22,
      (0.32 * oppositionIntensity) - (character.stats.intellect / 650)
    );
    playerCasualties = Math.min(playerTroops, Math.round(playerTroops * Math.min(0.52, basePlayerLossRate)));

    // Enemy still suffers attrition
    const baseEnemyLossRate = Math.max(0.08, 0.12 + (character.stats.martial / 600));
    enemyCasualties = Math.min(enemyTroops, Math.round(enemyTroops * Math.min(0.25, baseEnemyLossRate)));
  }
