...
import { Character, Province, Realm, Vassal, FamilyMember, LeaderProfile, RealmNPC, ProvincialSoldier } from '../types';
...
import { ProvincialBarracksModal } from './provinces/ProvincialBarracksModal';
import { 
  Castle, 
  Coins, 
  Shield, 
...
export const RealmProvincesOverlay: React.FC<RealmProvincesOverlayProps> = ({
  ...
  onEmployNPCAsVassal,
  onKnightSoldier = (_s, _n, _h) => {},
  onPromoteSoldier = (_sId) => {},
  onDemoteSoldier = (_sId) => {},
  onMakeProvincialHead = (_s) => {},
  onImprisonSoldier = (_sId) => {},
  onReleaseSoldier = (_sId, _o) => {},
  onExecuteSoldier = (_sId) => {},
  onUpgradeSoldierGear = (_sId, _c) => {}
}) => {
  ...
  const [activeBarracksProvince, setActiveBarracksProvince] = useState<Province | null>(null);
  ...
  const isFullyConquered = realmProvinces.length > 0 && playerControlledInRealm.length === realmProvinces.length;
  const isMajorityConquered = realmProvinces.length > 0 && playerControlledInRealm.length >= Math.ceil(realmProvinces.length * 0.5);
  const isPlayerHomeRealm = realm.id === character.realmId;
  const isPlayerSovereign = isPlayerHomeRealm || isFullyConquered || (isMajorityConquered && character.rank === 'Emperor');

  const canClaimEmperor = realmControlPercent >= 50 && character.rank !== 'Emperor';

  // Helper to clean and display governor title accurately
  const getDisplayGovernorName = (prov: Province) => {
    if (!prov.isPlayerControlled) {
      return prov.governorName || 'Provincial Lord';
    }
    if (!prov.governorName) return 'Imperial Castellan';
    const cleaned = prov.governorName
      .replace(/^(High Chieftain\s*(of the Packs Alpha)?|Arch-King|Grand Duke|Supreme Patriarch|Matron-Queen|Sovereign|Empress|Emperor)\s+/i, '')
      .trim();
    return `Crown Governor ${cleaned || prov.governorName}`;
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/85 backdrop-blur-sm animate-fade-in font-sans overflow-y-auto">
          ...
          {!selectedProvinceId ? (
            <div className="space-y-4">
              ...
              <div className="bg-stone-900 text-stone-100 p-4 rounded-2xl border border-stone-800 shadow-md">
                ...
                {/* CLICKABLE REALM LEADER / SOVEREIGN SECTION */}
                <div 
                  onClick={isPlayerSovereign ? undefined : handleOpenRealmLeaderProfile}
                  className={`p-3.5 rounded-xl border flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-all ${
                    isPlayerSovereign
                      ? 'bg-gradient-to-r from-amber-950/90 via-stone-950/95 to-amber-950/90 border-amber-500/60 shadow-lg'
                      : 'bg-stone-950/90 border-amber-500/30 hover:border-amber-400 hover:bg-stone-950 cursor-pointer group'
                  }`}
                  title={isPlayerSovereign ? 'Your Imperial Domain & Sovereign Crown' : 'Click to view full Leader Profile and interact diplomatically!'}
                >
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <span className="text-3xl block">
                        {isPlayerSovereign ? (character.portrait || '👑') : realm.leaderPortrait}
                      </span>
                      <span className={`absolute -bottom-1 -right-1 text-[9px] font-black px-1.5 rounded-full ${
                        isPlayerSovereign ? 'bg-amber-400 text-stone-950' : 'bg-amber-500 text-stone-950'
                      }`}>
                        {isPlayerSovereign ? 'YOU' : 'INFO'}
                      </span>
                    </div>
                    <div>
                      <div className="text-xs font-bold text-stone-100 flex items-center gap-1.5 flex-wrap">
                        {isPlayerSovereign ? (
                          <>
                            <span className="text-amber-300 font-cinzel text-sm font-black">
                              {character.rank} {character.name}
                            </span>
                            <span className="text-[10px] text-stone-950 bg-amber-400 px-1.5 py-0.5 rounded font-black font-sans shadow-2xs">
                              👑 Supreme Sovereign (You)
                            </span>
                          </>
                        ) : (
                          <>
                      ...
                      {isPlayerSovereign ? (
                        <div className="text-[11px] text-amber-200/90 mt-1 flex items-center gap-2 flex-wrap">
                          <span>Dynasty: <strong className="text-white">House {character.dynastyName}</strong></span>
                          <span>•</span>
                          <span className="text-emerald-400 font-bold">
                            {isFullyConquered ? '100% Annexed Sovereign Territory' : `Imperial Hegemony (${realmControlPercent}% Controlled)`}
                          </span>
                          {!isPlayerHomeRealm && (
                            <>
                              <span>•</span>
                              <span className="text-stone-400">
                                Subjugated Viceroy: <em>{realm.leaderName}</em>
                              </span>
                            </>
                          )}
                        </div>
                      ...
                      <>
                        <button
                          onClick={() => {
                            sound.playFanfare();
                            onAddChronicle({
                              title: `👑 Sovereign Crown Decrees Enacted in ${realm.name}`,
                              description: `As undisputed sovereign ruler of ${realm.name}, your imperial edicts resonate throughout the realm (+20 Renown).`,
                              type: 'diplomacy'
                            });
                            onUpdateCharacter({
                              stats: {
                          ...
                          className="px-2.5 py-1 rounded-lg bg-amber-600 hover:bg-amber-500 text-stone-950 text-xs font-black cursor-pointer flex items-center gap-1 shadow-xs"
                          title="Enact royal crown decrees across this sovereign realm"
                        >
                        ...
                        {!isPlayerHomeRealm && (
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg bg-amber-950/80 hover:bg-amber-900 text-amber-200 text-xs font-bold border border-amber-700/80 cursor-pointer flex items-center gap-1 shadow-xs"
                            title="Inspect the subjugated former leader of this domain"
                          >
                            <UserCheck className="w-3 h-3 text-amber-400" />
                            <span>Inspect Subjugated Noble</span>
                          </button>
                        )}
                        <button
                          onClick={() => { sound.playClick(); setActiveTab('realm_laws'); }}
                ...
                <div className="mt-3 bg-stone-950/80 p-3 rounded-xl border border-stone-800">
                  ...
                  <div className="w-full h-2.5 bg-stone-800 rounded-full overflow-hidden mb-2">
                    <div
                      className={`h-full transition-all rounded-full ${
                        isFullyConquered 
                          ? 'bg-gradient-to-r from-emerald-500 via-amber-400 to-yellow-300 shadow-sm'
                          : realmControlPercent >= 50 
                  ...
                  {/* STATUS AND PROMOTION DISPLAY */}
                  {isFullyConquered ? (
                    <div className="bg-gradient-to-r from-amber-950 via-yellow-950 to-amber-950 p-3 rounded-xl border border-amber-500/80 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-lg">
                      <div className="flex items-center gap-2.5">
                        <Crown className="w-6 h-6 text-yellow-400 animate-bounce shrink-0" />
                        <div>
                          <div className="text-xs font-extrabold text-yellow-300 font-cinzel flex items-center gap-2">
                            <span>🏆 TOTAL IMPERIAL CONQUEST (100%)</span>
                            <span className="bg-emerald-500 text-stone-950 text-[9px] font-black px-1.5 py-0.2 rounded font-sans">
                              SOVEREIGN RULER
                            </span>
                          </div>
                          <div className="text-[11px] text-amber-200">
                            All {realmProvinces.length} chartered counties are under your direct imperial crown. You are the undisputed Sovereign of {realm.name}!
                          </div>
                        </div>
                      </div>
                      {canClaimEmperor && (
                  ...
                  ) : isMajorityConquered && character.rank === 'Emperor' ? (
                    <div className="text-[11px] text-amber-200 flex items-center justify-between">
                      <span>
                        Current Status: <strong className="text-yellow-300 font-cinzel">Emperor & Sovereign Overlord</strong> ({playerControlledInRealm.length}/{realmProvinces.length} Provinces Held)
                      </span>
                      <span className="text-stone-300">
                        Annex remaining {foreignInRealm.length} counties for 100% conquest
                      </span>
                    </div>
                  ) : (
                    <div className="text-[11px] text-stone-400 flex items-center justify-between">
                      <span>
                        {playerControlledInRealm.length === 0 ? (
                          <span>Current Status: <strong className="text-stone-300">Foreign Domain</strong> (0 Provinces Held)</span>
                        ) : playerControlledInRealm.length <= 1 ? (
                          <span>Current Status: <strong className="text-stone-200">Vassal Count</strong> (1 Province)</span>
                        ) : (
                          <span>Current Status: <strong className="text-amber-300">King / Sovereign</strong> ({playerControlledInRealm.length} Provinces)</span>
                        )}
                      </span>
                    ...
                    {selectedProvince.isPlayerControlled ? (
                      <div className="flex items-center gap-2 flex-wrap">
                        <button
                          onClick={() => {
                            sound.playClick();
                            setActiveBarracksProvince(selectedProvince);
                          }}
                          className="px-3 py-1.5 rounded-xl bg-red-950 hover:bg-red-900 border border-red-800 text-red-200 text-xs font-bold flex items-center gap-1.5 shadow-md cursor-pointer transition-all active:scale-95"
                          title="Inspect provincial garrison, knight low-born soldiers, promote officers, or imprison"
                        >
                          <Swords className="w-3.5 h-3.5 text-red-400" />
                          <span>Barracks Garrison (20)</span>
                        </button>
                        <button
                          ...
                          <div key={bKey} className="p-3 bg-white rounded-xl border border-stone-300 shadow-xs flex flex-col justify-between gap-2">
                            ...
                            {key === 'barracks' && (
                              <button
                                ...
                                className="w-full py-1.5 rounded-lg text-xs font-bold bg-red-950 hover:bg-red-900 text-red-200 border border-red-800 flex items-center justify-center gap-1.5 shadow-xs cursor-pointer transition-colors"
                              >
                                <Swords className="w-3.5 h-3.5 text-red-400" />
                                <span>Inspect Barracks Garrison (Top 20 Soldiers)</span>
                              </button>
                            )}
                          </div>
                        );
      ...
      {/* Provincial Barracks & Top 20 Soldiers Modal */}
      {activeBarracksProvince && (
        <ProvincialBarracksModal
          province={activeBarracksProvince}
          character={character}
          soldiers={provincialSoldiers[activeBarracksProvince.id] || []}
          onClose={() => setActiveBarracksProvince(null)}
          onKnightSoldier={onKnightSoldier}
          onPromoteSoldier={onPromoteSoldier}
          onDemoteSoldier={onDemoteSoldier}
          onMakeProvincialHead={onMakeProvincialHead}
          onImprisonSoldier={onImprisonSoldier}
          onReleaseSoldier={onReleaseSoldier}
          onExecuteSoldier={onExecuteSoldier}
          onUpgradeSoldierGear={onUpgradeSoldierGear}
          onGrantProvinceToSoldier={(soldier) => {
            setActiveBarracksProvince(null);
