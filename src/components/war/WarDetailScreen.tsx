...
export const WarDetailScreen: React.FC<WarDetailScreenProps> = ({
  ...
  // Helper to format troop counts cleanly (e.g. 450 or 51.3k)
  const formatArmyNumber = (num: number) => {
    if (!num) return '0';
  ...
  const initialTroopSetting = war.yearlyTroops || war.playerLevies || Math.min(totalImperialTroops, 450);
  const [tempYearlyTroops, setTempYearlyTroops] = useState<number>(initialTroopSetting);
  ...
  const formattedImperialArmy = formatArmyNumber(totalImperialTroops);
  ...
  const annualLevyCount = war.playerLevies || war.yearlyTroops || 0;
  const annualUpkeepGold = Math.max(1, Math.round(annualLevyCount * 0.003));
    ...
    : [
        {
          ...
          assignedTroops: Math.round(annualLevyCount * 0.35),
          status: 'Engaged'
        },
        {
          id: 'cmd_2',
          ...
          assignedTroops: Math.round(annualLevyCount * 0.25),
          status: 'Ready'
        },
        {
          id: 'cmd_3',
          ...
          assignedTroops: Math.round(annualLevyCount * 0.15),
          status: 'Ready'
        },
        {
          id: 'cmd_4',
        ...
        },
        {
          ...
          assignedTroops: Math.round(annualLevyCount * 0.10),
          status: 'Ready'
        }
      ];
  ...
  const handleSaveYearlyTroops = () => {
    ...
    const updated: WarState = {
      ...
      playerLevies: tempYearlyTroops,
      commanders: commanders.map((c, i) => ({
        ...c,
        assignedTroops: Math.round(tempYearlyTroops * [0.35, 0.25, 0.15, 0.15, 0.10][i])
      }))
    };
    onUpdateWar(updated);
        ...
        <div>
          ...
          >
            <div className="flex items-center gap-3 min-w-0">
              <span className="text-2xl shrink-0">{war.isDefensive ? '🛡️' : '⚔️'}</span>
              <div className="min-w-0">
                <div className="font-black text-sm text-stone-950 truncate flex items-center gap-2">
                  <span>{war.title}</span>
                  {war.isDefensive ? (
                    <span className="text-[10px] font-black px-1.5 py-0.5 rounded bg-red-100 text-red-900 border border-red-400 shrink-0">
                      🛡️ Homeland Defense
                    </span>
                  ) : (
                    <span className="text-[10px] font-black px-1.5 py-0.5 rounded bg-amber-100 text-amber-900 border border-amber-300 shrink-0">
                      ⚔️ Expeditionary
                    </span>
                  )}
                </div>
                {war.claimUsed && (
                  <div className="text-[10px] text-stone-600 truncate font-semibold italic">
                    Goal: {war.claimUsed}
                  </div>
                )}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-extrabold text-sm text-stone-900">
                    Choose Yearly Army ({formatArmyNumber(war.yearlyTroops || war.playerLevies || 0)})
                  </div>
            ...
            <div className="bg-[#EFEAD4] p-4 rounded-2xl border border-stone-300 mb-5 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono font-black text-lg text-stone-950">
                  {formatArmyNumber(tempYearlyTroops)} Soldiers
                </span>
              </div>

              <input
                type="range"
                min={Math.max(10, Math.round(totalImperialTroops * 0.1))}
                max={Math.max(50, totalImperialTroops)}
                step={totalImperialTroops >= 1000 ? 50 : 1}
                value={tempYearlyTroops}
                onChange={(e) => setTempYearlyTroops(parseInt(e.target.value, 10))}
              ...
              <div className="flex justify-between text-[10px] text-stone-600 font-bold">
                <span>{formatArmyNumber(Math.max(10, Math.round(totalImperialTroops * 0.1)))} (Vanguard)</span>
                <span>{formatArmyNumber(Math.round(totalImperialTroops * 0.5))} (Standard)</span>
                <span>{formatArmyNumber(totalImperialTroops)} (All Banners)</span>
              </div>
              ...
              <div className="bg-amber-100/80 border border-amber-300 p-2.5 rounded-xl flex items-center justify-between text-xs font-bold text-amber-950">
                ...
                <span className="font-mono font-black">
                  -{Math.max(1, Math.round(tempYearlyTroops * 0.003))} Gold / yr
                </span>
              </div>
            </div>
