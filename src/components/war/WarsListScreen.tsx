              ...
              <div className="px-4 py-2 bg-[#EFEAD4] border-b border-stone-300 flex items-center justify-between text-xs font-bold text-stone-800">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-amber-950">
                    🪙 Total Upkeep: -{activeWars.reduce((sum, w) => sum + Math.round(((w.playerLevies || w.yearlyTroops) || 0) * 0.003), 0)} Gold/yr
                  </span>
                </div>
              </div>

              {activeWars.map((war) => {
                const warUpkeep = Math.round(((war.playerLevies || war.yearlyTroops) || 0) * 0.003);
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="font-extrabold text-sm sm:text-base text-stone-950 truncate flex flex-wrap items-center gap-2">
                          <span>{war.title}</span>
                          {war.isDefensive ? (
                            <span className="text-[10px] font-black px-1.5 py-0.5 rounded bg-red-100 text-red-900 border border-red-400 shrink-0 flex items-center gap-1">
                              🛡️ DEFENSIVE INVASION
                            </span>
                          ) : (
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-[11px]">
                          ...
                          <span className="text-stone-500 font-semibold">
                            • Forces: {((war.playerLevies || war.yearlyTroops || 0) / 1000).toFixed(1)}k vs {((war.enemyLevies || 0) / 1000).toFixed(1)}k • Year {war.warYear || 1} • Upkeep: -{warUpkeep} 🪙/yr
                          </span>
