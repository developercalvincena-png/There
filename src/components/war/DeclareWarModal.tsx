...
export const DeclareWarModal: React.FC<DeclareWarModalProps> = ({
  ...
  const rawMaxTroops = Math.max(10, totalImperialTroops);
  const initialCommitted = Math.max(10, Math.min(rawMaxTroops, Math.round(rawMaxTroops * 0.75)));
  const [committedTroops, setCommittedTroops] = useState<number>(initialCommitted);

  const availableClaims = targetType === 'realm' ? [
    {
      id: 'De Jure Imperial Reclamation',
      title: '🛡️ De Jure Imperial Reclamation',
      desc: `Assert ancient historical sovereignty over ${targetName}. Legitimate ancestral claim (-25% War Score required to enforce unconditional peace)!`
    },
    {
    ...
    {
      id: 'De Jure County Right',
      title: '🛡️ De Jure County Right',
      desc: 'Enforce ancient historical charter right. Requires 25% lower War Score to enforce complete annexation.'
    },
    {
  ...
  const handleFinalize = (commandDirectly: boolean) => {
    sound.playSword();
    onConfirmDeclaration(selectedClaim, committedTroops, commandDirectly);
  };
        ...
        <div className="bg-[#FAF7EB] border-2 border-[#D49B28] rounded-3xl max-w-sm sm:max-w-md w-full p-6 text-stone-900 shadow-2xl animate-fade-in font-sans">
          ...
          <div className="mb-6 bg-[#EFEAD4]/70 p-4 rounded-2xl border border-stone-300">
            ...
            <div className="flex justify-between text-[10px] text-stone-600 font-bold mt-1">
              <span>{rawMaxTroops >= 1000 ? `${(Math.max(10, Math.round(rawMaxTroops * 0.1)) / 1000).toFixed(1)}k` : Math.max(10, Math.round(rawMaxTroops * 0.1))} (Vanguard)</span>
              <span>{rawMaxTroops >= 1000 ? `${(Math.round(rawMaxTroops * 0.5) / 1000).toFixed(1)}k` : Math.round(rawMaxTroops * 0.5)} (Standard)</span>
              <span>{rawMaxTroops >= 1000 ? `${(rawMaxTroops / 1000).toFixed(1)}k` : rawMaxTroops} (Full Host)</span>
            </div>
          </div>
