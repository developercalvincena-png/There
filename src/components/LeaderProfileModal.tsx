...
interface LeaderProfileModalProps {
  ...
  onAddChronicle?: (entry: { title: string; description: string; type: 'diplomacy' | 'war' | 'intrigue' | 'feudal' | 'dynasty' }) => void;
}
...
export const LeaderProfileModal: React.FC<LeaderProfileModalProps> = ({
  ...
  onAddChronicle = (_entry: { title: string; description: string; type: 'diplomacy' | 'war' | 'intrigue' | 'feudal' | 'dynasty' }) => {}
}) => {
  ...
  const [localOpinion, setLocalOpinion] = useState<number>(leader?.opinion || 0);

  // Update local opinion when leader changes
  React.useEffect(() => {
    if (leader) setLocalOpinion(leader.opinion);
  }, [leader]);

  if (!isOpen || !leader) return null;

  const isLeaderControlled = isPlayerControlled || 
    leader.title.toLowerCase().includes('crown governor') || 
    leader.traits.some(t => t.toLowerCase().includes('crown subject') || t.toLowerCase().includes('loyal imperial'));
        ...
        <div className="bg-gradient-to-r from-[#2e1d0c] via-[#452b12] to-[#2e1d0c] p-4 border-b border-amber-500/40 relative">
          ...
          <div className="mt-3 bg-stone-950/70 p-2 rounded-xl border border-stone-800 flex items-center justify-between text-xs">
            <div className="flex items-center gap-2">
              ...
              <span className={`font-bold font-mono ${localOpinion >= 50 ? 'text-emerald-400' : localOpinion >= 0 ? 'text-blue-400' : 'text-red-400'}`}>
                {localOpinion > 0 ? `+${localOpinion}` : localOpinion} / 100
              </span>
            </div>
        ...
        <div className="p-4 overflow-y-auto space-y-3.5 flex-1">
          ...
          {activeTab === 'military' && (
            <div className="space-y-3">
              ...
              {/* Declare War Direct or Vassal Tithe/Governance */}
              {isLeaderControlled ? (
                <div className="bg-amber-950/40 p-3.5 rounded-xl border border-amber-500/50 space-y-2.5">
                  <div className="text-xs font-bold text-amber-300 flex items-center gap-1.5">
                    <Crown className="w-4 h-4 text-amber-400" />
                    <span>Crown Domain Subject & Vassal Governance</span>
                  </div>
                  <p className="text-[11px] text-stone-300">
                    {leader.name} serves directly under your sovereign crown. You cannot declare war upon your own loyal domain.
                  </p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
                    <button
                      onClick={() => {
                        sound.playCoin();
                        onAddChronicle({
                          title: `🪙 Imperial Tithe Collected from ${leader.name}`,
                          description: `Demanded special crown levies and gold contribution from ${leader.name}'s jurisdiction (+40 Gold).`,
                          type: 'feudal'
                        });
                        onUpdateCharacter({
                          stats: {
                        ...
                        setLocalOpinion(prev => Math.max(-100, prev - 10));
                        showFeedback(`Collected 40 gold in imperial tithes from ${leader.name}.`, 'success');
                      }}
                      className="py-2 px-3 rounded-lg bg-amber-900/80 hover:bg-amber-800 text-amber-200 font-bold text-xs border border-amber-600 flex items-center justify-center gap-1.5 cursor-pointer shadow-xs"
                    >
                      <span>🪙 Collect Imperial Tithe (+40 🪙)</span>
                    </button>
                    <button
                      onClick={() => {
                        sound.playFanfare();
                        onAddChronicle({
                          title: `🎖️ Crown Honors Bestowed on ${leader.name}`,
                          description: `Granted royal commendations and fief privileges to ${leader.name} (+25 Opinion, +15 Renown).`,
                          type: 'diplomacy'
                        });
                        onUpdateCharacter({
                          stats: {
                            ...playerCharacter.stats,
                            gold: Math.max(0, playerCharacter.stats.gold - 20),
                            renown: playerCharacter.stats.renown + 15
                          }
                        });
                        setLocalOpinion(prev => Math.min(100, prev + 25));
                        showFeedback(`Bestowed royal honors upon ${leader.name} (+25 Opinion).`, 'success');
                      }}
                      disabled={playerCharacter.stats.gold < 20}
                      className="py-2 px-3 rounded-lg bg-emerald-950 hover:bg-emerald-900 text-emerald-200 font-bold text-xs border border-emerald-700 flex items-center justify-center gap-1.5 disabled:opacity-40 cursor-pointer shadow-xs"
                    >
                      <span>🎖️ Bestow Royal Honors (-20 🪙)</span>
                    </button>
                  </div>
                </div>
              ) : (
