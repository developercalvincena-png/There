...
import { Character, FamilyMember, Province, Realm, RealmNPC, Species, Vassal } from '../types';
...
import { HeirTrainingTab } from './dynasty/HeirTrainingTab';
import { DynastyLegaciesScreen } from './dynasty/DynastyLegaciesScreen';
...
type ActiveView = 'relations_list' | 'heir_training' | 'character_detail' | 'family_tree' | 'past_relationships' | 'dynasty_legacies';
...
export const DynastyFamilyTab: React.FC<DynastyFamilyTabProps> = ({
  ...
  if (activeView === 'heir_training') {
    return (
      <HeirTrainingTab
        ...
        realmNPCs={realmNPCs}
        vassals={vassals}
        onUpdateFamilyMember={handleUpdateMember}
        onUpdateCharacter={onUpdateCharacter}
        onAddChronicle={onAddChronicle}
        onBackToFamily={() => setActiveView('relations_list')}
      />
    );
  }
  ...
  if (activeView === 'dynasty_legacies') {
    return (
      <div className="max-w-4xl mx-auto pb-16 font-sans">
        <button
          onClick={() => setActiveView('relations_list')}
          className="mb-4 px-3 py-1.5 bg-stone-800 hover:bg-stone-700 text-stone-200 text-xs font-serif rounded-lg flex items-center gap-1.5"
        >
          <span>‹ Back to Dynastic Relations</span>
        </button>
        <DynastyLegaciesScreen
          character={character}
          familyMembers={familyMembers}
          provinces={provinces}
          ...
          onCreateCadetBranch={onCreateCadetBranch}
          onBack={() => setActiveView('relations_list')}
        />
      </div>
    );
      ...
      <div className="bg-[#FAF7EB] text-[#181512] rounded-2xl border border-stone-400/60 shadow-2xl overflow-hidden">
        ...
        <div className="bg-[#D49B28] px-4 py-2.5 border-t border-b border-[#B78722] flex items-center justify-between text-[#181512] shadow-sm">
          ...
          <div className="flex items-center gap-1.5 flex-wrap justify-end">
            <button
              onClick={() => { sound.playClick(); setActiveView('dynasty_legacies'); }}
              className="px-2.5 py-1 bg-amber-950 text-amber-200 hover:bg-stone-900 rounded-lg text-xs font-black flex items-center gap-1 transition-all shadow-xs cursor-pointer border border-amber-600/50"
            >
              <span>👑 Legacies</span>
            </button>
            <button
              onClick={() => { sound.playClick(); setActiveView('heir_training'); }}
              className="px-2.5 py-1 bg-[#FAF7EB]/90 hover:bg-white text-stone-950 rounded-lg text-xs font-black flex items-center gap-1 transition-all shadow-xs cursor-pointer border border-[#B78722]"
            >
              <span>🎓 Training</span>
            </button>
            <button
              ...
              className="px-2 py-1 bg-[#FAF7EB]/40 hover:bg-[#FAF7EB]/70 rounded-lg text-xs font-bold text-stone-950 flex items-center gap-1 transition-all cursor-pointer"
            >
              <span>💍 Match</span>
            </button>
          </div>
        ...
        <div className="divide-y divide-stone-200">
          ...
          {/* SECTION: ACTIONS */}
          <div>
            {renderCategoryHeader('Actions & Mentorship')}
            <div className="divide-y divide-stone-200">
              {/* Heir Training */}
              <div
                onClick={() => { sound.playClick(); setActiveView('heir_training'); }}
                className="px-4 py-3 bg-[#FFF9E6] hover:bg-[#FCEEBE] transition-colors flex items-center justify-between cursor-pointer group border-l-4 border-amber-600"
              >
                <div className="flex items-center gap-3">
                  <span className="text-xl">🎓</span>
                  <div>
                    <div className="font-extrabold text-xs sm:text-sm text-amber-950 flex items-center gap-1.5">
                      <span>Heir Training & Royal Tutors</span>
                      <span className="text-[10px] px-1.5 py-0.2 rounded bg-amber-600 text-white font-bold">New</span>
                    </div>
                    <div className="text-[11px] text-stone-600">
                      Assign court mentors to children for Martial, Intellect & Diplomacy stat bonuses
                    </div>
                  </div>
                </div>
                <span className="text-amber-700 font-bold text-lg group-hover:translate-x-1 transition-transform">›</span>
              </div>
