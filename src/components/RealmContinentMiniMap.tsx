          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 truncate">
                ...
                <span className={`text-[9.5px] font-medium ${pCount === rProvs.length && rProvs.length > 0 ? 'text-amber-300 font-bold' : 'text-stone-400'}`}>
                  {pCount === rProvs.length && rProvs.length > 0 ? '👑 Fully Conquered' : pCount > 0 ? `${pCount}/${rProvs.length} Fiefs` : `${rProvs.length} Counties`}
                </span>
              </div>
            </div>
          );
