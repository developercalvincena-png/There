...
export const WarDiplomacyTab: React.FC<WarDiplomacyTabProps> = ({
  ...
  // Filter out realms where ALL provinces are already player-controlled
  const foreignRealms = realms.filter(r => {
    if (r.id === character.realmId) return false;
    const rProvs = provinces.filter(p => p.realmId === r.id);
    if (rProvs.length > 0 && rProvs.every(p => p.isPlayerControlled)) {
      return false; // Already completely conquered by player
    }
    return true;
  });
  ...
  const allTargetEntities: TargetEntity[] = [
    ...
    ...foreignRealms.map(r => {
      const realmTroops = r.armyStrength || (r.militaryPower * 10) || 14000;
      return {
        id: r.id,
        name: r.name,
        type: 'realm' as const,
        species: r.species,
        troops: realmTroops,
        maxTroops: Math.round(realmTroops * 1.4),
        leaderName: `${r.leaderTitle} ${r.leaderName}`,
        leaderAge: r.leaderProfile?.age || 45,
