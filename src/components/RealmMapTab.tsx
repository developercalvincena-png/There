...
import { Character, Province, Realm, Vassal, FamilyMember, RealmNPC, ProvincialSoldier } from '../types';
        ...
        <div className="space-y-4">
          ...
          <div className="bg-stone-900/90 rounded-2xl p-4 sm:p-5 border border-stone-800 shadow-xl">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 border-b border-stone-800 pb-3">
              ...
              {/* Quick Actions: Grant Province, Invest, Festival, Barracks Garrison */}
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-red-950/90 hover:bg-red-900 border border-red-700/80 text-red-200 text-xs font-bold flex items-center gap-1.5 shadow-md cursor-pointer transition-all active:scale-95"
                  title="Inspect provincial garrison barracks, knight low-born commoners, promote officers, or imprison mutineers"
                >
                  <Swords className="w-3.5 h-3.5 text-red-400" />
                  <span>Barracks Garrison (Top 20)</span>
                </button>
                <button
                  onClick={() => {
                    sound.playClick();
              ...
              {BUILDINGS_CONFIG.map((bConfig) => {
                ...
                return (
                  ...
                  >
                    ...
                    {bConfig.key === 'barracks' && (
                      <button
                        onClick={() => {
                          sound.playClick();
                          setActiveBarracksProvince(selectedProvince);
                        }}
                        className="w-full py-1.5 px-2.5 rounded-lg bg-red-950/70 hover:bg-red-900/90 border border-red-700/60 text-red-200 text-[11px] font-bold flex items-center justify-center gap-1.5 shadow-xs cursor-pointer transition-colors"
                      >
                        <Swords className="w-3.5 h-3.5 text-red-400" />
      ...
      {activeBarracksProvince && (
        <ProvincialBarracksModal
          ...
          onGrantProvinceToSoldier={(soldier) => {
            ...
            setProvinceToGrant(activeBarracksProvince);
          }}
        />
      )}
