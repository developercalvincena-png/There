...
export const VassalsSystemModal: React.FC<VassalsSystemModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-sm overflow-hidden font-sans">
      ...
      {/* 13. Family View Modal with Detailed Children & Relatives */}
      {activeActionModal === 'family_view' && currentVassal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-md bg-[#FEF9E7] rounded-2xl p-5 border-2 border-amber-800 shadow-2xl text-stone-950 space-y-4 max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between border-b border-amber-900/20 pb-2">
              <h3 className="font-black text-lg text-stone-900 flex items-center gap-2">
                <span>👨‍👩‍👧</span> House of {currentVassal.houseName || 'Percival'} Dynastic Family
              </h3>
              <button onClick={() => setActiveActionModal(null)} className="p-1 hover:bg-amber-200 rounded-full">
                <X className="w-5 h-5" />
              </button>
            ...
            <div className="space-y-3 text-xs text-stone-800 overflow-y-auto flex-1 pr-1">
              
              {/* Spouse Card */}
              <div className="p-3 bg-amber-100/90 rounded-xl border border-amber-800/20 flex items-center gap-3">
                <span className="text-2xl">{currentVassal.familyTree?.spouse?.portrait || '👰'}</span>
                <div className="flex-1">
                  <div className="text-[10px] text-stone-600 uppercase font-black">Noble Spouse / Consort</div>
                  <div className="text-sm font-black text-stone-950">
                    {currentVassal.familyTree?.spouse?.name || currentVassal.family?.spouseName || 'Lady Gwilym Percival'} 
                    {currentVassal.familyTree?.spouse ? ` (Age ${currentVassal.familyTree.spouse.age})` : ''}
                  </div>
                  <div className="text-[11px] text-stone-700">
                    {currentVassal.familyTree?.spouse?.traits?.join(' • ') || 'Noble Lineage • Chaste'}
                  </div>
                </div>
              </div>

              {/* Designated Heir Card */}
              <div className="p-3 bg-amber-200/80 rounded-xl border border-amber-800/30 flex items-center gap-3">
                <span className="text-2xl">{currentVassal.familyTree?.designatedHeir?.portrait || '🤴'}</span>
                <div className="flex-1">
                  <div className="text-[10px] text-amber-900 uppercase font-black flex items-center gap-1">
                    <span>👑</span> Designated Heir to the County
                  </div>
                  <div className="text-sm font-black text-stone-950">
                    {currentVassal.familyTree?.designatedHeir?.name || currentVassal.family?.heirName || 'Maredudd Percival'} 
                    {currentVassal.familyTree?.designatedHeir ? ` (Age ${currentVassal.familyTree.designatedHeir.age})` : ' (Age 14)'}
                  </div>
                  <div className="text-[11px] text-stone-700">
                    {currentVassal.familyTree?.designatedHeir?.traits?.join(' • ') || 'Quick • Martial Training'}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <div className="text-[11px] font-black uppercase text-stone-700 px-1">
                  Offspring & Heirs ({currentVassal.familyTree?.children?.length || currentVassal.family?.childrenCount || 2})
                </div>

                {currentVassal.familyTree?.children && currentVassal.familyTree.children.length > 0 ? (
                  currentVassal.familyTree.children.map((child) => (
                    <div key={child.id} className="p-2.5 bg-amber-50 rounded-xl border border-amber-800/15 flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        <span className="text-xl">{child.portrait || (child.gender === 'Male' ? '👦' : '👧')}</span>
                        <div>
                          <div className="font-bold text-stone-950 text-xs">
                            {child.name} ({child.gender}, Age {child.age})
                          </div>
                          <div className="text-[10px] text-stone-600">
                            {child.traits?.join(', ') || 'Healthy'} {child.isBetrothed ? '• 💍 Betrothed' : '• Unbetrothed'}
                          </div>
                        </div>
                      </div>
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-200 text-stone-900 font-extrabold border border-amber-700/20">
                        Health: {child.health}%
                      </span>
                    </div>
                  ))
                ) : (
                  <div className="p-2 bg-amber-50 rounded-xl border border-amber-800/10 text-stone-600 text-xs italic text-center">
                    2 young heirs being tutored in castle keep.
                  </div>
                )}
              </div>

              {/* Hostage Notice if held */}
              {currentVassal.isHostage && (
                <div className="p-2.5 bg-red-100 rounded-xl border border-red-400 text-red-900 text-xs font-bold flex items-center gap-2">
                  <span>⛓️</span>
                  <span>Currently held as a political ward/hostage in imperial custody, guaranteeing treaty peace.</span>
                </div>
              )}
            </div>
